      ...
      <a className="instagram-frame" href={post.permalink} target="_blank" rel="noopener noreferrer"
        ...
        <div className="instagram-photo">
          {post.displayUrl && !imageFailed ? <img src={post.displayUrl} alt={post.caption?.slice(0, 160) ?? 'From the bestbvnnies Instagram feed'} loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={() => setImageFailed(true)} /> :
            <div className="instagram-image-fallback"><span aria-hidden="true">✳</span><p>Catch this one<br />on Instagram.</p></div>}
        </div>
        <span className="instagram-photo-label"><span>{label}</span><span aria-hidden="true">↗</span></span>
      </a>
...
export function InstagramSection() {
  ...
  return <section className="instagram-section" id="instagram" aria-labelledby="instagram-title">
    ...
    <div className="instagram-heading">
      ...
      <div className="instagram-intro"><p>The sets. The details.<br />The very best bits.</p><span className="instagram-sticker" aria-hidden="true">LOOKS GOOD<br />ON YOU. ✳</span></div>
    </div>
    {feed.status === 'loading' && <div className="instagram-loading" role="status"><p>Pinning up the latest…</p><div className="instagram-placeholders" aria-hidden="true">{[0, 1, 2].map((i) => <span key={i}>✳</span>)}</div></div>}
    ...
    {(feed.status === 'unavailable' || (feed.status === 'ready' && !feed.posts.length)) && <div className="instagram-unavailable" role="status"><span aria-hidden="true">✳</span><div><h3>More good stuff over there.</h3><p>{feed.status === 'unavailable' ? 'The feed is taking a moment. Find the latest on Instagram.' : 'Our next photo diary is coming. Find us on Instagram in the meantime.'}</p></div></div>}
    <div className="instagram-follow"><p>Stay in the loop.<br /><strong>Come hang with the bvnnies.</strong></p><a className="instagram-follow-link" href={profileUrl} target="_blank" rel="noopener noreferrer">Follow @bestbvnnies <span aria-hidden="true">↗</span><span className="sr-only"> on Instagram (opens in a new tab)</span></a></div>
  </section>
}
